...
- 최신 커밋: 전달받은 zip 파일 안에는 `.git` 폴더가 없어(용량·보안을 위해 `git archive`가
  일부러 뺀다) 압축을 푼 폴더 안에서 `git log`로 커밋 해시를 직접 확인할 수는 없다. **그래서
  버전이 맞는지는 커밋 해시가 아니라 아래 "버전 확인" 절의 grep/ls 커맨드로 판단한다 —
  이 커맨드가 실질적으로 가장 정확한 검증 수단이다.**
- 문서: `docs/PRD.md` (제품 요구사항 전체), 이 파일(`docs/HANDOFF.md`)
- 코드 번들 `task-auto-making-source.zip`의 SHA256 (참고용 무결성 확인):
  `0fe56f391044a5c9d4c68ed01d40f9fe0aad3216cd642b589386b401ccdbe942`
  이 값은 커밋 `95428c7dfdc591a3f2d9db7fa58d0f2856dbd16c`(run.sh와 이 안내문 초안이 막
  추가된 시점)에서 `git archive`로 만든 zip 기준이다. **주의**: 이 문서(HANDOFF.md) 자신이
  매번 만들어지는 zip 안에 포함되기 때문에, "이 문서에 적힌 SHA256 값"이 "지금 전달받은 zip
  파일 자체의 SHA256"과 정의상 완벽히 같을 수는 없다(이 문단을 다듬어 적어 넣는 행위 자체가
  그다음 커밋이 되어버리기 때문 — 자기 자신을 포함하는 파일이 자기 자신의 해시값을 정확히
  담을 수 없는 것과 같은 이유다). 실제 전달되는 zip은 이 값보다 문서 정리용 커밋 한두 개가
  더 반영되어 있을 수 있고, 코드 내용은 완전히 동일하다. 그래서 **전송 중 파일이 손상되지
  않았는지 확인하는 용도로만** 이 값을 참고하고(값이 위 값과 "비슷한 시점"인지 정도), 실제
  "버전이 최신인지"는 반드시 아래 "버전 확인" 절 커맨드로 판단할 것. 지금 실제로 전달되는
  파일의 정확한 SHA256 값은 이 zip과 함께 전달자가 채팅/전달 메시지로 다시 한번 알려준다.
...
```bash
sha256sum task-auto-making-source.zip   # 전달자가 알려준 값과 같은지 확인 (파일 손상 여부)
...
**진짜 버전 확인은 아래 두 줄(`grep -c`, `ls run.sh`)로 한다.** `grep -c`가 3이 아니거나
`run.sh`가 없으면 옛날 버전이니 다시 받아야 한다. `sha256sum`은 "전달 도중 파일이 깨지지
않았는지"를 보는 용도이고, 정확한 비교값은 이 문서 위쪽 값이 아니라 **전달자가 이 zip을 줄
때 채팅/메시지로 같이 알려준 값**과 비교한다 (문서에 적힌 값은 문서 자신을 포함하는 zip의
해시를 문서 자신이 완벽히 미리 알 수는 없다는 한계로, 실제 전달 파일과 한두 커밋 정도
차이가 날 수 있음 — 위 "최신 커밋" 항목 설명 참고). "이전 버전 아니냐"는 의심이 들 때는
느낌으로 판단하지 말고 반드시 `grep -c` / `ls run.sh` 결과 숫자로 확인할 것.
